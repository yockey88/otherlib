  ...
  canonical_operand canonical_instruction::lower_argument(const normalized_operand& operand) {
    switch (operand.kind) {
      case operand_kind::ADDRESS_U16:
        ...
        return { .kind = operand_kind::ADDRESS_U16, .val = operand.address.value() };
      case operand_kind::INTEGER_LITERAL:
        ...
        return { .kind = operand_kind::IMMEDIATE_U16, .val = operand.constant.value() };
      case operand_kind::FLOAT_LITERAL:
        return { .kind = operand_kind::FLOAT_LITERAL, .bytes = operand.bytes };
      case operand_kind::STRING_LITERAL:
        return { .kind = operand_kind::STRING_LITERAL, .bytes = operand.bytes };
      case operand_kind::CODE_LABEL:
        if (!operand.symbol.has_value()) {
          throw ocmd_lowering_error("CODE_LABEL operand is missing symbol");
        }
        return { .kind = operand_kind::CODE_LABEL, .symbol = operand.symbol.value() };
      case operand_kind::DATA_SYMBOL:
        if (!operand.symbol.has_value()) {
          throw ocmd_lowering_error("DATA_SYMBOL operand is missing symbol");
        }
        return { .kind = operand_kind::DATA_SYMBOL, .symbol = operand.symbol.value() };
      case operand_kind::REGISTER_REF:
        ...
        return { .kind = operand_kind::REGISTER_REF, .reg = operand.reg.value() };
      default:
