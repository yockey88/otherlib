...
namespace other {
  ...
  class driver_kernel {
   public:
    ...
    template <typename T>
      requires std::derived_from<T, core_system<T>>
    T& initialize_builtin_system(driver_system_type type) {
      OTHER_ASSERT(type < kNumBuiltinDriverSystems, "Invalid builtin system type: {}", static_cast<uint32_t>(type));
      T* system = arena_allocator<T>{}.allocate(driver_instance);
      OTHER_ASSERT(system != nullptr, "Failed to allocate builtin system of type {}", typeid(T).name());
    ...
    template <typename T>
      requires std::derived_from<T, core_system<T>>
    void shutdown_builtin_system(driver_system_type type) {
      OTHER_ASSERT(type < kNumBuiltinDriverSystems, "Invalid builtin system type: {}", static_cast<uint32_t>(type));
