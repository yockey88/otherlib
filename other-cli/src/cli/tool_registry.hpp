...
namespace other {
  namespace cli {
    ...
    /// name -> tool table shared by the standalone exe and in-environment hosts; unique_ptr not
    ///  other::scope because scope<> needs the arena subsystem and cli must run without engine boot
    class tool_registry {
     public:
    ...
    /// process-wide registry with builtin project-workflow tools (create, open) pre-registered;
    ///  developer hosts layer the source-tree workflow on top via register_dev_tools
    ...
    /// convenience entry points for invoking tools from inside a running environment; the
    ///  context-less overload prints to stdout/stderr and locates the environment from the current process
