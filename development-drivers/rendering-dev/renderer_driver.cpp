...
namespace other {
  ...
  void renderer_driver::on_initialize(const command_line& cmd) {
    ...
    get_event_system()->trigger_event("force-load-empty-scene", std::string("Renderer Dev Scene"));

    {
      PROFILE_SECTION("renderer_driver::on_initialize--scene-setup");
      scene_object& light_obj = get_active_scene()->create_object("Light", glm::vec3(0.f, 2.f, 0.f));
      get_active_scene()->add_object_tag(light_obj.id, "scene-ambient-light");

      scene_object& cam_obj = get_active_scene()->create_object("Main Camera", glm::vec3(0.f, 0.f, 0.f));
      get_active_scene()->add_object_tag(cam_obj.id, "main-camera");

      other::camera& cam = get_active_scene()->add_component<camera>(&cam_obj);
      ...
      transform& light_transform = get_active_scene()->get_transform(&light_obj);
...

      {
        ...
        suzanne_asset_id = get_asset_manager()->load_asset(model_path);
      }

      scene_object& suzanne_obj = get_active_scene()->create_object("Suzanne", glm::vec3(0.f, -0.5f, 0.f));
      script_component* suzanne_script = get_active_scene()->get_component<script_component>(&suzanne_obj);
      ...
      mouse.position = get_renderer_instance().get_mouse_position();
  ...
  void renderer_driver::run() {
    ...
    while (running) {
      ...
      get_asset_manager()->update_pipelines();

      if (!loaded_suzanne && get_asset_manager()->get_asset_state(suzanne_asset_id) == asset_state::LOADED) {
        uint64_t hash = get_asset_manager()->get_asset_hash(suzanne_asset_id);
        ref<model_source> suzanne_source = subsystem<renderer_backend>::get()->get_model_source(hash);
        if (suzanne_source != nullptr) {
          scene_object& suzanne_obj = get_active_scene()->get_object(suzanne_id);
          suzanne_model = suzanne_source->produce_model("Suzanne");

          render_component& suzanne_render = get_active_scene()->add_component<render_component>(&suzanne_obj);
          suzanne_render.animated = true;
          suzanne_render.model_asset_id = suzanne_asset_id;
          suzanne_render.obj_model = suzanne_model;
          ...
          //   anim_ctrl.model_ptr = &suzanne_render.obj_model;
      ...
      if (pressing_mouse_wheel) {
        ...
        scene_object& cam_obj = get_active_scene()->get_object(camera_id);
        camera* cam = get_active_scene()->get_component<camera>(&cam_obj);
      ...
      /// update animation
      {
        ...
        get_active_scene()->update(delta_time);
        get_active_scene()->late_update(delta_time);
      }

      {
        PROFILE_SECTION("rendering-dev--render-frame");
        render_data scene_render_data = get_active_scene()->prepare_render_data(get_asset_manager());

        get_renderer_instance().begin_frame(&scene_render_data);
        ...
        auto* transform_comp = get_active_scene()->get_component<transform>(suzanne_id);
        auto* render_comp = get_active_scene()->get_component<render_component>(suzanne_id);
        if (render_comp != nullptr) {
          if (ImGui::Begin("Debug Window")) {
            ...
            if (ImGui::DragFloat3("Light Position", glm::value_ptr(get_active_scene()->get_component<gpu::point_light>(light_id)->light_position), 0.1f)) {}
            if (ImGui::DragFloat3("Light Color", glm::value_ptr(get_active_scene()->get_component<gpu::point_light>(light_id)->color), 0.01f, 0.f, 1.0f)) {}
